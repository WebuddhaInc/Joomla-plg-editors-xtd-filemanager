...
if (!defined('_JDEFINES')) {
	$_path = explode(DIRECTORY_SEPARATOR, $_SERVER['SCRIPT_FILENAME']);
	$_path = implode(DIRECTORY_SEPARATOR, array_slice($_path, 0, count($_path) - 5));
	define('JPATH_ROOT', $_path);
	define('JPATH_BASE', $_path . DIRECTORY_SEPARATOR . 'administrator');
	unset( $_path );
...
$base_url    = explode('/', JURI::root());
$base_url    = implode('/', array_slice($base_url, 0, count($base_url) - 4));
...
$config = array(
	...
	*/
	...
	|
	*/
	'upload_dir' => '/' . $mediaConfig->get('file_path') . '/',
	...
	|
	*/
	'current_path' => JPATH_ROOT . '/' . $mediaConfig->get('file_path') . '/',
	...
	*/
	...
	|
	*/
	'thumbs_base_path' => JPATH_ROOT . '/media/thumbs/',
